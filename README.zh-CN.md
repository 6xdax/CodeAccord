...
Explore 每次都以相同的紧凑结构结束：实施就绪状态、变更类型、目标、当前事实、实施方案、修改范围、兼容性与风险、验收标准和开放决策。小改动也保留这些标题，但每节可以只有一句。

`确认后可实施` 表示用户确认后可以直接开始修改；`仍有决策阻塞` 必须列出剩余选择，不能作为完整实施范围请求确认。
...
即使直接实施，Agent 也必须先完成只读 Explore，并在修改前给出固定格式的 Accord；此时可以不再等待一次回复。
...
Hook 不负责总结 transcript。命令 Hook 无法可靠推断 Agent 尚未保存的决策，而且 transcript 格式不是稳定契约。因此，Skill 要求 Agent 主动更新语义内容，Hook 只提供检查和恢复入口。
